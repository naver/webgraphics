...
#if USE_WEBKIT_CHANGES
    case PatternType::CAIRO:
      {
        PatternCairo *cairoPat = static_cast<PatternCairo*>(mOrigPattern);
        mPattern =
          new (mCairoPat) PatternCairo(cairoPat->mPattern);
        return mPattern;
      }
    case PatternType::DIRECT2D:
      {
        PatternD2D *d2dPat = static_cast<PatternD2D*>(mOrigPattern);
        mPattern =
          new (mD2DPat) PatternD2D(d2dPat->mBrush);
        return mPattern;
      }
#endif
    default:
...
#if USE_WEBKIT_CHANGES
    char mCairoPat[sizeof(PatternCairo)];
    char mD2DPat[sizeof(PatternD2D)];
#endif
  };

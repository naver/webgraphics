...
#if USE_WEBKIT_CHANGES
bool
DrawTargetD2D1::Init(ID3D11Texture2D* aTexture, ID2D1DeviceContext* aDC, ID2D1Bitmap1* aBitmap, SurfaceFormat aFormat)
{
  HRESULT hr;
...
#if USE_WEBKIT_CHANGES
void
DrawTargetD2D1::SetFactory(ID2D1Factory1 * factory)
{
  if (mFactory) {
    mFactory->Release();
    mFactory = nullptr;
...
#if USE_WEBKIT_CHANGES
  if (iter != CurrentLayer().mPushedClips.end()) {
#endif
      if (iter->mPath) {
...
#if USE_WEBKIT_CHANGES
IntRect DrawTargetD2D1::GetCurrentClipBounds()
{
    IntRect bounds;
...
{
#if USE_WEBKIT_CHANGES
  if (aPattern.GetType() == PatternType::DIRECT2D) {
    PatternD2D pattern = static_cast<const PatternD2D&>(aPattern);
    return pattern.mBrush.forget();
  }
#endif

...
ScaledFontDWrite::GetPathForGlyphs(const GlyphBuffer &aBuffer, const DrawTarget *aTarget)
{
  ...
  if (aTarget->IsRecording()) {
    PathBuilderRecording *pathBuilderRecording =
      static_cast<PathBuilderRecording*>(pathBuilder.get());
    pathBuilderD2D = static_cast<PathBuilderD2D*>(pathBuilderRecording->GetFinalPathBuilder());
  } else
    pathBuilderD2D = 
#endif

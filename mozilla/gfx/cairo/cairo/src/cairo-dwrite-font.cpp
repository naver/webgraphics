			    ...
			    uint32_t		      ucs4)
{
    ...
    UINT16 index;
    face->dwriteface->GetGlyphIndices(&ucs4, 1, &index); //wasling GetGlyphIndicesA -> GetGlyphIndices
    return index;
}
...
#if USE_WEBKIT_CHANGES
static cairo_bool_t
_cairo_dwrite_font_face_from_hdc (HDC hdc, void **font_face)
{
    RefPtr<IDWriteGdiInterop> gdiInterop;
    DWriteFactory::Instance()->GetGdiInterop (&gdiInterop);

    HRESULT hr = gdiInterop->CreateFontFaceFromHdc (hdc, reinterpret_cast<IDWriteFontFace**>(font_face));
    return SUCCEEDED(hr);
}
...
_cairo_dwrite_font_face_create_for_hdc (HDC hdc)
{
    ...
    return cairo_dwrite_font_face_create_for_dwrite_fontface (nullptr, dwrite_font_face);
}
...
cairo_dwrite_font_face_create_for_hfont (HFONT font)
{
    ...
    cairo_font_face_t* font_face = _cairo_dwrite_font_face_create_for_hdc (hdc);
...
cairo_font_face_t *
cairo_dwrite_font_face_create_for_font_file (void *font_file, int face_type)
{
    IDWriteFontFile *ff = static_cast<IDWriteFontFile*>(font_file);

    RefPtr<IDWriteFontFace> font_face;
    if (FAILED(DWriteFactory::Instance ()->CreateFontFace (static_cast<DWRITE_FONT_FACE_TYPE>(face_type), 1, &ff, 0, DWRITE_FONT_SIMULATIONS_NONE, &font_face))) {
        return nullptr;
    }
    return cairo_dwrite_font_face_create_for_dwrite_fontface (nullptr, font_face);
}

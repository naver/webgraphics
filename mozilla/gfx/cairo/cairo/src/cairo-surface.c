		     ...
		     cairo_content_t			 content)
{
    ...
    surface->gpu_surface = NULL;
    surface->gpu_surface_destroy = NULL;
    surface->destroy_observer = NULL;
#endif
}
...
#if USE_WEBKIT_CHANGES
    if (surface->gpu_surface)
    surface->gpu_surface_destroy(surface->gpu_surface);

    if (surface->destroy_observer)
    surface->destroy_observer(surface);
#endif
    free (surface);
}
...
cairo_surface_set_device_scale (cairo_surface_t *surface,
				...
				double		 y_scale)
{
    _cairo_surface_set_device_scale (surface, x_scale, y_scale);
}

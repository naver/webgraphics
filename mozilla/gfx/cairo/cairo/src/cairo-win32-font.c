...
#if USE_WEBKIT_CHANGES
    if (font_face->dwrite_font) {
        cairo_font_face_destroy(font_face->dwrite_font);
    }
#endif
}
...
#if USE_WEBKIT_CHANGES
cairo_font_face_t *
cairo_dwrite_font_face_create_for_win32_font (cairo_font_face_t *font)
{
    if (font->backend->type != CAIRO_FONT_TYPE_WIN32) {
	_cairo_error_throw (CAIRO_STATUS_FONT_TYPE_MISMATCH);
    return NULL;
    }

    cairo_win32_font_face_t *font_face = (cairo_win32_font_face_t *)font;
    if (font_face->dwrite_font) {
        return font_face->dwrite_font;
    }

    font_face->dwrite_font = cairo_dwrite_font_face_create_for_hfont(font_face->hfont);
    if (!font_face->dwrite_font) {

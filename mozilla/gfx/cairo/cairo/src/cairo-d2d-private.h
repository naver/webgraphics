...
#else
#include "cairo-d2d-private-d3d11-fx.h"
#endif
...
struct _cairo_d2d_device
{
    ...
    RefPtr<ID2D1Device> mD2D1Device;
    RefPtr<ID3DX11Effect> mSampleEffect;
    RefPtr<ID3D11InputLayout> mInputLayout;
    RefPtr<ID3D11Buffer> mQuadBuffer;
    RefPtr<ID3D11RasterizerState> mRasterizerState;
    RefPtr<ID3D11BlendState> mBlendStates[MAX_OPERATORS];
    /** Texture used for manual glyph rendering */
    RefPtr<ID3D11Texture2D> mTextTexture;
    RefPtr<ID3D11ShaderResourceView> mTextTextureView;
#endif
    int mVRAMUsage;
};
...
#else
    RefPtr<ID3D11Texture2D> bufferTexture;
#endif
...
#else
    RefPtr<ID3D11RenderTargetView> buffer_rt_view;
    RefPtr<ID3D11ShaderResourceView> buffer_sr_view;
#endif
...
#else
typedef PFN_D3D11_CREATE_DEVICE D3D11CreateDeviceFunc;
#endif

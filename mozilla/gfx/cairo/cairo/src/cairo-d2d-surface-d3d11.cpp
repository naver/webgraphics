...
static ID2D1Factory1 *sD2DFactory = NULL;
            ...
            cairo_operator_t op)
{
    ...
    device->mDeviceContext->OMSetBlendState(device->mBlendStates[op], NULL, 0xffffffff);
    ...
    case CAIRO_OPERATOR_IN:
        ...
        desc.RenderTarget[0].SrcBlend = desc.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_DEST_ALPHA;
        break;
    case CAIRO_OPERATOR_OUT:
        desc.RenderTarget[0].BlendOp = desc.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;
        desc.RenderTarget[0].DestBlend = desc.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_ZERO;
        desc.RenderTarget[0].SrcBlend = desc.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_INV_DEST_ALPHA;
        break;
    ...
    case CAIRO_OPERATOR_DEST_IN:
        desc.RenderTarget[0].BlendOp = desc.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;
        desc.RenderTarget[0].DestBlend = desc.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_SRC_ALPHA;
        desc.RenderTarget[0].SrcBlend = desc.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ZERO;
        break;
    ...
    device->mD3D11Device->CreateBlendState(&desc, &device->mBlendStates[op]);
...
cairo_device_t *
cairo_d2d_create_device_from_d3d11device(ID3D11Device *d3d11device)
{
    HRESULT hr;
    ...
    CD3D11_TEXTURE2D_DESC textDesc(DXGI_FORMAT_B8G8R8A8_UNORM,
                   TEXT_TEXTURE_WIDTH,
                   TEXT_TEXTURE_HEIGHT,
                   1, 1);
    ...
    memset(&device->base, 0, sizeof(cairo_device_t));
#endif
    device->mD3D11Device = d3d11device;
    device->mD3D11Device->GetImmediateContext(&device->mDeviceContext);

    device->mD3D11 = LoadLibraryA("d3d11.dll");
...
#endif
    hr = createD2DFactory(D2D1_FACTORY_TYPE_MULTI_THREADED,
    ...
    device->mD3D11Device->QueryInterface(&dxgiDevice);
    hr = sD2DFactory->CreateDevice(dxgiDevice, &device->mD2D1Device);
    if (FAILED(hr)) {
    goto FAILED;
    }

    D3D11_PRIMITIVE_TOPOLOGY originalTopology;
    device->mDeviceContext->IAGetPrimitiveTopology(&originalTopology);
    device->mDeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINESTRIP);
    D3DX11CreateEffectFromMemory((void*)g_main, sizeof(g_main), 0, device->mD3D11Device, &device->mSampleEffect);
    device->mDeviceContext->IASetPrimitiveTopology(originalTopology);

    technique = device->mSampleEffect->GetTechniqueByName("SampleTexture");
    technique->GetPassByIndex(0)->GetDesc(&passDesc);


    hr = device->mD3D11Device->CreateInputLayout(layout,
    ...
    hr = device->mD3D11Device->CreateBuffer(&bufferDesc, &data, &device->mQuadBuffer);
    if (FAILED(hr)) {
    goto FAILED;
    }
    ...
    hr = device->mD3D11Device->CreateRasterizerState(&rastDesc, &device->mRasterizerState);
    if (FAILED(hr)) {
    goto FAILED;
    }
    device->base.refcount = 1;
#if USE_WEBKIT_CHANGES
    device->base.type = CAIRO_DEVICE_TYPE_D2D;
#endif
    ...
    hr = device->mD3D11Device->CreateTexture2D(&textDesc, NULL, &device->mTextTexture);
    if (FAILED(hr)) {
    goto FAILED;
    }

    hr = device->mD3D11Device->CreateShaderResourceView(device->mTextTexture,
                            NULL,
...
cairo_d2d_create_device()
{
    ...
    D3D11CreateDeviceFunc createD3DDevice = (D3D11CreateDeviceFunc)
    ...
    /**
     * On usage of D3D11_CREATE_DEVICE_PREVENT_INTERNAL_THREADING_OPTIMIZATIONS:
     * documentation on D3D11_CREATE_DEVICE_PREVENT_INTERNAL_THREADING_OPTIMIZATIONS
    ...
    cairo_device_t *device = cairo_d2d_create_device_from_d3d11device(d3ddevice);
...
#if USE_WEBKIT_CHANGES
struct ID3D11Device *
cairo_d2d_get_d3d11device_from_device(cairo_device_t *device)
{
    cairo_d2d_device_t *d2d_device = reinterpret_cast<cairo_d2d_device_t*>(device);
    return d2d_device->mD3D11Device;
}

struct ID2D1Factory1 *
cairo_d2d_get_factory1()
{
    return sD2DFactory;
}

struct ID2D1Device *
cairo_d2d_get_d2d1device_from_device(cairo_device_t *device)
{
    cairo_d2d_device_t *d2d_device = reinterpret_cast<cairo_d2d_device_t*>(device);
    return d2d_device->mD2D1Device;
}
#endif
...
cairo_release_device(cairo_device_t *device)
{
    ...
    cairo_d2d_device_t *d2d_device = reinterpret_cast<cairo_d2d_device_t*>(device);
    HMODULE d3d11 = d2d_device->mD3D11;
    delete d2d_device;
    _cairo_d2d_release_factory();
    FreeLibrary(d3d11);
    }
...
cairo_d2d_finish_device(cairo_device_t *device)
{
    ...
    d2d_device->mD3D11Device->CreateQuery(&queryDesc, &query);

    d2d_device->mDeviceContext->End(query);
    ...
    if (FAILED(d2d_device->mDeviceContext->GetData(query, &done, sizeof(BOOL), 0))) {
        break;
    }
    if (FAILED(d2d_device->mD3D11Device->GetDeviceRemovedReason())) {
        break;
    }
    }
}
...
{
    device->mDeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
    device->mDeviceContext->IASetInputLayout(device->mInputLayout);

    UINT stride = sizeof(Vertex);
    UINT offset = 0;
    auto *buff = device->mQuadBuffer.get();
    device->mDeviceContext->IASetVertexBuffers(0, 1, &buff, &stride, &offset);

    device->mDeviceContext->RSSetState(device->mRasterizerState);
}
...
{
    int size = surface->dc->GetPixelSize().width * surface->dc->GetPixelSize().height;
    size *= surface->dc->GetPixelFormat().format == DXGI_FORMAT_A8_UNORM ? 1 : 4;
    return size;
}
...
_cairo_d2d_get_buffer_texture(cairo_d2d_surface_t *surface) 
{
    ...
    CD3D11_TEXTURE2D_DESC softDesc(surfDesc.Format, surfDesc.Width, surfDesc.Height);
    softDesc.MipLevels = 1;
    softDesc.Usage = D3D11_USAGE_DEFAULT;
    softDesc.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE;
    surface->device->mD3D11Device->CreateTexture2D(&softDesc, NULL, &surface->bufferTexture);
...
{
    if (!d2dsurf->backBuf && d2dsurf->dc->GetPixelFormat().format != DXGI_FORMAT_A8_UNORM) {
    return;
    }
    
    if (!d2dsurf->surfaceBitmap) {
    d2dsurf->dc->CreateBitmap(d2dsurf->dc->GetPixelSize(),
                  D2D1::BitmapProperties(d2dsurf->dc->GetPixelFormat()),
                  &d2dsurf->surfaceBitmap);
    }

    d2dsurf->surfaceBitmap->CopyFromBitmap(NULL, d2dsurf->bitmap, NULL);
}
...
void cairo_d2d_present_backbuffer(cairo_surface_t *surface)
{
    ...
    d2dsurf->device->mDeviceContext->Flush();
    }
}
...
push_clip (cairo_d2d_surface_t *d2dsurf, cairo_clip_path_t *clip_path)
{
    ...
    d2dsurf->dc->PushAxisAlignedClip (
        D2D1::RectF(
    ...
    hr = d2dsurf->dc->CreateLayer (&layer);
    ...
    hr = d2dsurf->dc->QueryInterface(IID_ID2D1DeviceContext, (void**)&dc);

    if (FAILED(hr)) {
        d2dsurf->dc->PushLayer(D2D1::LayerParameters(
                       D2D1::InfiniteRect(),
                       geom,
                  ...
                  int *x, int *y, unsigned int *width, unsigned int *height)
{
    ...
    UINT32 maxSize = d2dsurf->dc->GetMaximumBitmapSize() - 2;
                    ...
                    cairo_radial_pattern_t *source_pattern)
{
    ...
    source_pattern->r1 != 0) {
#if USE_WEBKIT_CHANGES
        d2d_can_handle = false;
    } else if (source_pattern->r1 == source_pattern->r2) {
        cairo_pattern_t * color_pattern = cairo_pattern_create_rgba(0.0, 0.0, 0.0, 0.0);
        if (!color_pattern)
            return NULL;

        RefPtr<ID2D1Brush> brush = _cairo_d2d_create_brush_for_pattern(d2dsurf, NULL, color_pattern, false);

        cairo_pattern_destroy(color_pattern);
        return brush;
    }

    if (source_pattern->r1 > source_pattern->r2) {
        d2d_can_handle = false;
        reversed = true;
    } 
    ...
    if (!d2d_can_handle) {
        /**
         * Create an image surface with the gradient and use that.
         * \todo Upstream!
         */
        double x0, y0, z0;
        double x1, y1, z1;
        if (reversed) {
            if (cairo_pattern_get_radial_circles(&source_pattern->base.base, &x1, &y1, &z1, &x0, &y0, &z0) != CAIRO_STATUS_SUCCESS)
                return NULL;
        } else {
            if (cairo_pattern_get_radial_circles(&source_pattern->base.base, &x0, &y0, &z0, &x1, &y1, &z1) != CAIRO_STATUS_SUCCESS)
                return NULL;
        }

        double l0 = x0 - z0, l1 = x1 - z1;
        double t0 = y0 - z0, t1 = y1 - z1;
        double r0 = x0 + z0, r1 = x1 + z1;
        double b0 = y0 + z0, b1 = y1 + z1;

        if (l0 - l1 < 0 || t0 - t1 < 0 || r1 - r0 < 0 || b1 - b0 < 0) {
            /**
             * We can't properly handle cone-shaped radial gradients.
             * \todo Fix this!
             */
            return NULL;
        }
        ...
        double l = MIN(l0, l1) - pad;
        double t = MIN(t0, t1) - pad;
        double r = MAX(r0, r1) + pad;
        double b = MAX(b0, b1) + pad;
        double w = r - l;
        double h = b - t;
        double tx = -l;
        double ty = -t;
        cairo_surface_t * source_surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, w, h);
        if (!source_surface)
            return NULL;

        cairo_t *cr = cairo_create(source_surface);

        cairo_translate(cr, tx, ty);
        cairo_rectangle(cr, l, t, w, h);
        cairo_set_source(cr, &source_pattern->base.base);
        cairo_fill(cr);
        cairo_destroy(cr);

        cairo_pattern_t * surface_pattern = cairo_pattern_create_for_surface(source_surface);
        if (!surface_pattern) {
        ...
        cairo_matrix_t mat;
        cairo_matrix_init_translate(&mat, tx, ty);
        cairo_pattern_set_matrix(surface_pattern, &mat);
        cairo_pattern_set_extend(surface_pattern, CAIRO_EXTEND_PAD);

        RefPtr<ID2D1Brush> brush = _cairo_d2d_create_brush_for_pattern(d2dsurf, NULL, surface_pattern, false);
    ...
    d2dsurf->dc->CreateGradientStopCollection(stops, num_stops, &stopCollection);
    RefPtr<ID2D1RadialGradientBrush> brush;

    d2dsurf->dc->CreateRadialGradientBrush(D2D1::RadialGradientBrushProperties(center,
                                           origin,
                                           outer_radius,
                                           outer_radius),
                       brushProps,
                    ...
                    cairo_linear_pattern_t *source_pattern)
{
    ...
    d2dsurf->dc->CreateSolidColorBrush(
    ...
    d2dsurf->dc->CreateLinearGradientBrush(D2D1::LinearGradientBrushProperties(D2D1::Point2F((FLOAT)p1.x, (FLOAT)p1.y),
                    ...
                    bool unique = false)
{
    ...
    if (unique) {
        RefPtr<ID2D1SolidColorBrush> brush;
        d2dsurf->dc->CreateSolidColorBrush(color,
                           &brush);
        return brush;
    } else {
    ...
    if (surfacePattern->surface->type == CAIRO_SURFACE_TYPE_D2D) {
        ...
        if (pattern->extend != CAIRO_EXTEND_NONE) {
            hr = d2dsurf->dc->CreateBitmap(D2D1::SizeU(width, height),
            ...
            hr = d2dsurf->dc->CreateBitmap(D2D1::SizeU(tmpWidth, tmpHeight),
                           tmp,
                           tmpWidth * Bpp,
                           D2D1::BitmapProperties(D2D1::PixelFormat(format,
    ...
    if (unique) {
        ...
        d2dsurf->dc->CreateBitmapBrush(sourceBitmap, 
                       &bitProps,
                       &brushProps,
                       &bitBrush);
        return bitBrush;
...
#if USE_WEBKIT_CHANGES
RefPtr<ID2D1Brush>
cairo_d2d_create_brush_for_pattern(cairo_d2d_surface_t *d2dsurf,
				    cairo_path_fixed_t *fill_path,
				    const cairo_pattern_t *pattern)
{
    return _cairo_d2d_create_brush_for_pattern(d2dsurf, fill_path, pattern, true);
}
#endif
...
#if USE_WEBKIT_CHANGES
RefPtr<ID2D1PathGeometry>
cairo_d2d_create_path_geometry_for_path(cairo_path_fixed_t *path, 
                     cairo_fill_rule_t fill_rule,
                     D2D1_FIGURE_BEGIN type)
{
    return _cairo_d2d_create_path_geometry_for_path(path, fill_rule, type);
}
#endif
         ...
         cairo_box_t *box)
{
    ...
    d2dsurf->dc->PushAxisAlignedClip(D2D1::RectF(_cairo_fixed_to_float(box->p1.x),
            _cairo_fixed_to_float(box->p1.y),
            _cairo_fixed_to_float(box->p2.x),
            _cairo_fixed_to_float(box->p2.y)),
        D2D1_ANTIALIAS_MODE_ALIASED);
    d2dsurf->dc->Clear(D2D1::ColorF(0, 0));
    d2dsurf->dc->PopAxisAlignedClip();
...
_cairo_d2d_surface_init(cairo_d2d_surface_t *newSurf, cairo_d2d_device_t *d2d_device, cairo_format_t format)
{
    ...
    newSurf->base.device = reinterpret_cast<cairo_device_t*>(d2d_device);
              ...
              int             height)
{
    ...
    CD3D11_TEXTURE2D_DESC desc(origDesc.Format,
                   sizePixels.width,
                   sizePixels.height);
    ...
    /* CreateTexture2D does not support D3D11_RESOURCE_MISC_GDI_COMPATIBLE with DXGI_FORMAT_A8_UNORM */
    ...
    D2D1_BITMAP_OPTIONS usage = (desc.MiscFlags & D3D11_RESOURCE_MISC_GDI_COMPATIBLE) ?
                      D2D1_BITMAP_OPTIONS_GDI_COMPATIBLE
                    : D2D1_BITMAP_OPTIONS_NONE;

    hr = d2dsurf->device->mD3D11Device->CreateTexture2D(&desc, NULL, &texture);
    if (FAILED(hr)) {
    ...
    hr = d2dsurf->device->mD2D1Device->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_ENABLE_MULTITHREADED_OPTIMIZATIONS, &newSurf->dc);
    ...
    props.dpiX = dpiX;
    props.dpiY = dpiY;
    props.pixelFormat = D2D1::PixelFormat(DXGI_FORMAT_UNKNOWN, alpha);
    props.colorContext = nullptr;
    hr = newSurf->dc->CreateBitmapFromDxgiSurface(dxgiSurface, props, &newSurf->bitmap);

    if (FAILED(hr)) {
    goto FAIL_CREATESIMILAR;
    }

    newSurf->dc->SetTarget(newSurf->bitmap);
    ...
    hr = newSurf->dc->CreateSharedBitmap(IID_IDXGISurface,
                         dxgiSurface,
                         &bitProps,
    ...
    newSurf->dc->CreateSolidColorBrush(D2D1::ColorF(0, 1.0), &newSurf->solidColorBrush);
...
#if USE_WEBKIT_CHANGES
cairo_surface_t*
cairo_d2d_create_similar(void            *surface,
              cairo_content_t     content,
              int             width,
              int             height)
{
    return _cairo_d2d_create_similar(surface, content, width, height);
}
#endif
                ...
                void                   **image_extra)
{
    ...
    HRESULT hr;
    D2D1_SIZE_U size = d2dsurf->dc->GetPixelSize();
     ...
     */
    softDesc.MipLevels = 1;
    softDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE | D3D11_CPU_ACCESS_READ;
    softDesc.Usage = D3D11_USAGE_STAGING;
    softDesc.BindFlags = 0;
    hr = d2dsurf->device->mD3D11Device->CreateTexture2D(&softDesc, NULL, &softTexture);
    if (FAILED(hr)) {
    return _cairo_error(CAIRO_STATUS_NO_MEMORY);
    }

    d2dsurf->device->mDeviceContext->CopyResource(softTexture, d2dsurf->surface);

    D3D11_MAPPED_SUBRESOURCE data;
    hr = d2dsurf->device->mDeviceContext->Map(softTexture, 0, D3D11_MAP_READ_WRITE, 0, &data);
                ...
                void                   *image_extra)
{
    ...
    ID3D11Texture2D *softTexture = (ID3D11Texture2D*)image_extra;
    
    d2dsurf->device->mDeviceContext->Unmap(softTexture, 0);
    softTexture->Release();
    softTexture = NULL;
}
                  ...
                  void                    *image_extra)
{
    ...
    d2dsurf->device->mDeviceContext->Unmap(softTexture, 0);
    d2dsurf->device->mDeviceContext->CopyResource(d2dsurf->surface, softTexture);
    softTexture->Release();
}
...
#if USE_WEBKIT_CHANGES
    if (d2dsurf->dt) {
    HRESULT hr = d2dsurf->dt->Flush();
    }
#endif
    if (d2dsurf->isDrawing) {
    reset_clip(d2dsurf);
    HRESULT hr = d2dsurf->dc->EndDraw();
            ...
            cairo_region_t *region)
{
    ...
    ID3D11Resource *srcResource = src->surface;
    if (src->surface.get() == dst->surface.get()) {
    // Self-copy
    srcResource = _cairo_d2d_get_buffer_texture(dst);
    src->device->mDeviceContext->CopyResource(srcResource, src->surface);
    } else {
    ...
    src->device->mDeviceContext->CopySubresourceRegion(dst->surface,
                             0,
                             area_to_copy.x,
                             area_to_copy.y,
                             0,
             ...
             float opacity)
{
    ...
    x1 = y1 = 0;
    x2 = dst->dc->GetSize().width;
    y2 = dst->dc->GetSize().height;
    }
    ...
    dst->dc->DrawBitmap(src->surfaceBitmap,
            rectDst,
            opacity,
            interpMode,
            rectSrc);
    if (needsTransform) {
    dst->dc->SetTransform(D2D1::IdentityMatrix());
    }
            ...
            float opacity = 1.0f)
{
    ...
    rect.x = rect.y = 0;
    rect.width = dst->dc->GetPixelSize().width;
    rect.height = dst->dc->GetPixelSize().height;
    }
    ...
    cairo_rectangle_int_t surface_rect = { 0, 0,
                           dst->dc->GetPixelSize().width,
                           dst->dc->GetPixelSize().height };
...
static RefPtr<ID2D1DeviceContext>
_cairo_d2d_get_temp_dc(cairo_d2d_surface_t *surf, cairo_clip_t *clip)
{
    RefPtr<ID3D11Texture2D> texture = _cairo_d2d_get_buffer_texture(surf);
    RefPtr<ID2D1DeviceContext> new_dc;
    ...
    hr = surf->device->mD2D1Device->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_ENABLE_MULTITHREADED_OPTIMIZATIONS, &new_dc);
    ...
    props.colorContext = nullptr;
    hr = new_dc->CreateBitmapFromDxgiSurface(dxgiSurface, props, &bitmap);

    if (FAILED(hr)) {
        return NULL;
    }

    new_dc->SetTarget(bitmap);

    new_dc->BeginDraw();
    new_dc->Clear(D2D1::ColorF(0, 0));
    ...
    while (path) {
        ...
        new_dc->PushAxisAlignedClip(D2D1::RectF(_cairo_fixed_to_float(clip_box.p1.x),
        ...
        hr = new_dc->CreateLayer (&layer);

        D2D1_LAYER_OPTIONS options = D2D1_LAYER_OPTIONS_NONE;

        new_dc->PushLayer(D2D1::LayerParameters(
                    D2D1::InfiniteRect(),
                    geom,
                    D2D1_ANTIALIAS_MODE_PER_PRIMITIVE,
                    D2D1::IdentityMatrix(),
...
static cairo_int_status_t
_cairo_d2d_blend_temp_surface(cairo_d2d_surface_t *surf, cairo_operator_t op, ID2D1DeviceContext *dc, cairo_clip_t *clip, const cairo_rectangle_int_t *bounds = NULL)
{
    ...
    RefPtr<ID3D11Texture2D> srcTexture = _cairo_d2d_get_buffer_texture(surf);
    RefPtr<ID3D11Texture2D> dstTexture;

    surf->surface->QueryInterface(&dstTexture);
    ID3D11Device *device = surf->device->mD3D11Device;
    ID3D11DeviceContext *deviceContext = surf->device->mDeviceContext;
    ...
    ID3DX11Effect *effect = surf->device->mSampleEffect;

    ID3D11RenderTargetView *rtViewPtr = surf->buffer_rt_view;
    deviceContext->OMSetRenderTargets(1, &rtViewPtr, 0);
    ID3DX11EffectVectorVariable *quadDesc = effect->GetVariableByName("QuadDesc")->AsVector();
    ID3DX11EffectVectorVariable *texCoords = effect->GetVariableByName("TexCoords")->AsVector();
    ...
    ID3DX11EffectTechnique *technique = effect->GetTechniqueByName("SampleTexture");
    technique->GetPassByIndex(0)->Apply(0, deviceContext);

    ID3D11ShaderResourceView *srViewPtr = surf->buffer_sr_view;
    deviceContext->PSSetShaderResources(0, 1, &srViewPtr);

    deviceContext->Draw(4, 0);

#ifdef DEBUG
    // Quiet down some info messages from D3D11 debug layer
    srViewPtr = NULL;
         ...
         cairo_clip_t        *clip)
{
    ...
    RefPtr<ID2D1DeviceContext> target_dc = d2dsurf->dc;
#ifndef ALWAYS_MANUAL_COMPOSITE
    if (op != CAIRO_OPERATOR_OVER) {
#endif
    target_dc = _cairo_d2d_get_temp_dc(d2dsurf, clip);
    if (!target_dc) {
        return CAIRO_INT_STATUS_UNSUPPORTED;
    }
#ifndef ALWAYS_MANUAL_COMPOSITE
    ...
    target_dc->SetAntialiasMode(D2D1_ANTIALIAS_MODE_ALIASED);
    ...
    D2D1_SIZE_F size = target_dc->GetSize();
    target_dc->FillRectangle(D2D1::RectF((FLOAT)0,
                     (FLOAT)0,
                     (FLOAT)size.width,
                     (FLOAT)size.height),
                 brush);

    if (target_dc.get() != d2dsurf->dc.get()) {
    return _cairo_d2d_blend_temp_surface(d2dsurf, op, target_dc, clip);
    }
        ...
        cairo_clip_t        *clip)
{
    ...
    d2dsurf->dc->CreateLayer(&d2dsurf->maskLayer);
    }
    target_dc->PushLayer(D2D1::LayerParameters(D2D1::InfiniteRect(),
                           0,
                           D2D1_ANTIALIAS_MODE_ALIASED,
                           D2D1::IdentityMatrix(),
                           1.0,
          ...
          cairo_clip_t        *clip)
{
    ...
    target_dc->SetAntialiasMode(D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
    }
    ...
    if (transformed) {
      ...
      target_dc->SetTransform(mat);
      d2dpath = trans_geom;
    } else {
      mat = D2D1::Matrix3x2F::Identity();
    }

    target_dc->DrawGeometry(d2dpath, brush, (FLOAT)style->line_width, strokeStyle);

    if (transformed) {
      target_dc->SetTransform(D2D1::Matrix3x2F::Identity());
    }
    ...
    return _cairo_d2d_blend_temp_surface(d2dsurf, op, target_dc, clip, &bound_rect);
    }
        ...
        cairo_clip_t        *clip)
{
    ...
    target_dc->FillRectangle(D2D1::RectF(x1,
                         y1,
                         x2,
                         y2),
                 brush);
    ...
    target_dc->FillGeometry(d2dpath, brush);
    }
    ...
    return _cairo_d2d_blend_temp_surface(d2dsurf, op, target_dc, clip, &bounds);
    }
                        ...
                        cairo_clip_t            *clip)
{
    ...
    dst->dc->GetTextRenderingParams(&params);
    ...
    ID3D11Device *device = dst->device->mD3D11Device;
    ID3D11DeviceContext *deviceContext = dst->device->mDeviceContext;
    ...
    RefPtr<ID3D11Texture2D> tmpTexture;
    ...
    hr = deviceContext->Map(tmpTexture, 0, D3D11_MAP_WRITE, 0, &texMap);
    ...
    deviceContext->Unmap(tmpTexture, 0);
    ...
    deviceContext->CopySubresourceRegion(dst->device->mTextTexture, 0, 0, 0, 0, tmpTexture, 0, &box);
    ...
    for (int y = 0; y < cairo_bounds.height; y++) {
        ...
        // texture creation. D3D11 has no 3 Bpp texture format we can
        // use.
        //
        // Since we don't care what ends up in the alpha pixel of the
        // destination, therefor we can simply copy a normal 32 bit
    ...
    ID3DX11Effect *effect = dst->device->mSampleEffect;

    ID3D11RenderTargetView *rtViewPtr = dst->buffer_rt_view;

    deviceContext->OMSetRenderTargets(1, &rtViewPtr, 0);

    ID3DX11EffectTechnique *technique = effect->GetTechniqueByName("SampleTextTexture");

    ID3DX11EffectVectorVariable *quadDesc = effect->GetVariableByName("QuadDesc")->AsVector();
    ID3DX11EffectVectorVariable *texCoords = effect->GetVariableByName("TexCoords")->AsVector();
    ID3DX11EffectVectorVariable *textColor = effect->GetVariableByName("TextColor")->AsVector();
    ...
    ID3D11ShaderResourceView *srViewPtr = srView;
    deviceContext->PSSetShaderResources(0, 1, &srViewPtr);
                     ...
                     cairo_clip_t        *clip)
{
    ...
    RefPtr<ID2D1DeviceContext> target_dc = dst->dc;
    cairo_rectangle_int_t fontArea;
#ifndef ALWAYS_MANUAL_COMPOSITE
    if (op != CAIRO_OPERATOR_OVER) {
#endif
    target_dc = _cairo_d2d_get_temp_dc(dst, clip);
    ...
    if (dst->base.content != CAIRO_CONTENT_COLOR || dst->dc.get() != target_dc.get()) {
    cleartype_quality = D2D1_TEXT_ANTIALIAS_MODE_GRAYSCALE;
    }

    RefPtr<IDWriteRenderingParams> params;
    target_dc->GetTextRenderingParams(&params);
    ...
    target_dc->SetTextAntialiasMode(cleartype_quality);
    ...
    target_dc->DrawGlyphRun(D2D1::Point2F(0, 0), &run, brush, measureMode);
    
    if (transform) {
    target_dc->SetTransform(D2D1::Matrix3x2F::Identity());
    }

    if (target_dc.get() != dst->dc.get()) {
    return _cairo_d2d_blend_temp_surface(dst, op, target_dc, clip, &fontArea);
    }
            ...
            int            *remaining_glyphs)
{
    ...
    d2dsurf->dc->SetTextRenderingParams(params);
                  ...
                  cairo_content_t content)
{
    ...
    ID3D11Device *device = d2d_device->mD3D11Device;
    RefPtr<IDXGIDevice> dxgiDevice;
    ...
    hr = d2d_device->mD2D1Device->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_ENABLE_MULTITHREADED_OPTIMIZATIONS, &newSurf->dc);

    if (FAILED(hr)) {
    goto FAIL_HWND;
    }

    props.bitmapOptions = D2D1_BITMAP_OPTIONS_TARGET | D2D1_BITMAP_OPTIONS_GDI_COMPATIBLE;
    props.pixelFormat = D2D1::PixelFormat(DXGI_FORMAT_UNKNOWN, D2D1_ALPHA_MODE_PREMULTIPLIED);
    props.dpiX = dpiX;
    props.dpiY = dpiY;
    props.colorContext = nullptr;
    hr = newSurf->dc->CreateBitmapFromDxgiSurface(newSurf->backBuf, props, &newSurf->bitmap);
             ...
             int height)
{
    ...
    hr = d2d_device->mD3D11Device->CreateTexture2D(&desc, NULL, &texture);
    ...
    if (desc.MiscFlags & D3D11_RESOURCE_MISC_GDI_COMPATIBLE)
        props.bitmapOptions |= D2D1_BITMAP_OPTIONS_GDI_COMPATIBLE;
    props.pixelFormat = D2D1::PixelFormat(DXGI_FORMAT_UNKNOWN, alpha);
    props.colorContext = nullptr;
    hr = newSurf->dc->CreateBitmapFromDxgiSurface(dxgiSurface, props, &newSurf->bitmap);
...
cairo_d2d_surface_create_for_handle(cairo_device_t *device, HANDLE handle, cairo_content_t content)
{
    ...
    hr = d2d_device->mD3D11Device->OpenSharedResource(handle,
                     ...
                     cairo_content_t content)
{
    ...
    newSurf->dc->GetDpi(&props.dpiX, &props.dpiY);
...
void cairo_d2d_scroll(cairo_surface_t *surface, int x, int y, cairo_rectangle_t *clip)
{
    ...
    ID3D11Texture2D *texture = _cairo_d2d_get_buffer_texture(d2dsurf);

    d2dsurf->device->mDeviceContext->CopyResource(texture, d2dsurf->surface);
    d2dsurf->device->mDeviceContext->CopySubresourceRegion(d2dsurf->surface,
                          0,
                          point.x,
                          point.y,
                          0,
...
cairo_d2d_get_dc(cairo_surface_t *surface, cairo_bool_t retain_contents)
{
     ...
     * interface */
    d2dsurf->dc->QueryInterface(&interopRT);

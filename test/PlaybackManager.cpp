...
DrawTarget* PlaybackManager::LookupDrawTarget(ReferencePtr aRefPtr)
{
...
Path* PlaybackManager::LookupPath(ReferencePtr aRefPtr)
{
...
FilterNode* PlaybackManager::LookupFilterNode(ReferencePtr aRefPtr)
{
...
SourceSurface* PlaybackManager::LookupSourceSurface(ReferencePtr aRefPtr)
{
...
GradientStops* PlaybackManager::LookupGradientStops(ReferencePtr aRefPtr)
{
...
ScaledFont* PlaybackManager::LookupScaledFont(ReferencePtr aRefPtr)
{
...
NativeFontResource *PlaybackManager::LookupNativeFontResource(uint64_t aKey)
{
    NativeFontResourceMap::iterator iter = mNativeFontResources.find(aKey);

    if (iter != mNativeFontResources.end()) {
        return iter->second;
    }
...
FontType PlaybackManager::GetDesiredFontType()
{
...
void PlaybackManager::AddNativeFontResource(uint64_t aKey, mozilla::gfx::NativeFontResource * aScaledFontResouce)
{
    mNativeFontResources[aKey] = aScaledFontResouce;
}

void PlaybackManager::PlaybackToEvent(int aID)
{
    PlayToEvent(aID);
}

void PlaybackManager::EventDisablingUpdated(int32_t _t1)
{
}

bool PlaybackManager::IsClipPush(uint32_t aID, int32_t aRefID)
{
    if (aRefID != -1 && mRecordedEvents[aID]->GetObjectRef() != mRecordedEvents[aRefID]->GetObjectRef()) {
        return false;
    }
    return mRecordedEvents[aID]->GetType() == RecordedEvent::PUSHCLIP || mRecordedEvents[aID]->GetType() == RecordedEvent::PUSHCLIPRECT;
}

bool PlaybackManager::IsClipPop(uint32_t aID, int32_t aRefID)
{
    if (aRefID != -1 && mRecordedEvents[aID]->GetObjectRef() != mRecordedEvents[aRefID]->GetObjectRef()) {
        return false;
    }
...
bool PlaybackManager::FindCorrespondingClipID(uint32_t aID, uint32_t *aOtherID)
{
    if (IsClipPush(aID)) {
...
void PlaybackManager::DisableEvent(uint32_t aID)
{
...
void PlaybackManager::EnableEvent(uint32_t aID)
{
...
void PlaybackManager::PlaybackEvent(RecordedEvent *aEvent)
{
    aEvent->PlayEvent(this);
}

double PlaybackManager::GetEventTiming(uint32_t aID, bool aAllowBatching, bool aIgnoreFirst, bool aDoFlush, bool aForceCompletion, double *aStdDev)
{
    if (!aID) {
        return 0;
    }
...
bool PlaybackManager::CanDisableEvent(RecordedEvent *aEvent)
{
...
bool PlaybackManager::DisableOneTimeEvent(uint32_t aID)
{
    RecordedEvent *event = mRecordedEvents[aID];
...
void PlaybackManager::ForceCompletion()
{
#ifdef WIN32
    if (mBaseDT->GetType() == (DrawTargetType)(BackendType::DIRECT2D)) {
